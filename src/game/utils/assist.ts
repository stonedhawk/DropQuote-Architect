...
const axisSteps: Record<Axis, { forward: { x: number; y: number }; backward: { x: number; y: number } }> = {
  horizontal: {
    forward: { x: 1, y: 0 },
    backward: { x: -1, y: 0 },
  },
...
export const createFunFirstPreview = (
  ...
  if (opportunity) {
    return {
      ...
      hint: `Fun-first assist: this ${opportunity.letter} is meant to help finish ${opportunity.targetWord}. Reverse words count too.`,
    }
  }

  const sequenceLetter =
    FUN_FIRST_SEQUENCE[assistCursor % FUN_FIRST_SEQUENCE.length]

  if (sequenceLetter) {
    const wordIndex = assistCursor % EASY_WORD_POOL.length
    const targetWord = EASY_WORD_POOL[wordIndex] ?? EASY_WORD_POOL[0] ?? 'CAT'
    return {
      letter: sequenceLetter,
      assistMode: 'sequence',
      targetWord,
      hint: `Fun-first assist is feeding easy letters for words like ${targetWord}. Reverse words count too.`,
    }
  }
